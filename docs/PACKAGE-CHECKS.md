...
| GitHub 仓库子路径 | 通过，`/migration-studio/` 下入口、预览和全部运行资源返回 200 |
...
npm test
npm run build
npm run preview -- --port 8788 --base /migration-studio/
```

最后一条可在本机模拟 GitHub 项目站点的子路径；打开 `http://127.0.0.1:8788/migration-studio/`。

浏览器自检需用开发模式 `npm start`，再打开 `/tests/smoke.html`；测试页不发布到 `dist/`。

## 仍需在目标环境确认

- 此文件记录上传前的本地打包检查；线上部署以仓库 Actions 的最新运行和实际访问结果为准。线上入口为 `https://liamgvchi.github.io/migration-studio/`。
